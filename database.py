...
def init_database():
    ...
    # Exchange rate settings - NEW TABLE
    c.execute('''CREATE TABLE IF NOT EXISTS exchange_rate (
        id INTEGER PRIMARY KEY AUTOINCREMENT,
        eur_to_usd REAL NOT NULL DEFAULT 1.16,
        updated_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP
    )''')
    ...
    existing_rate = c.execute('SELECT COUNT(*) FROM exchange_rate').fetchone()[0]
    if existing_rate == 0:
        c.execute('INSERT INTO exchange_rate (eur_to_usd) VALUES (?)', (1.16,))
...
def get_exchange_rate():
    ...
    rate = conn.execute('SELECT eur_to_usd FROM exchange_rate ORDER BY id DESC LIMIT 1').fetchone()
    conn.close()
...
def update_exchange_rate(new_rate):
    ...
    conn = get_db_connection()
    conn.execute('INSERT INTO exchange_rate (eur_to_usd) VALUES (?)', (new_rate,))
    conn.commit()
    conn.close()
